...
厳密には「X が**その先行ブロックの少なくとも一つを支配する**が、**X 自身はそのブロックを厳密には支配しない**ようなブロック」の集合だ
（X が自分自身の支配辺境に入ることもある——ループの入口がその典型）。
...
SCCP はこう進む——まず `x = 10`（定数）と分かる。すると `x > 0` は**真で確定**するので、
偽の枝の先 **B2 は到達不能**になり、点火されない。点火されない B2 の `y = 0` は合流に参加しないから、
